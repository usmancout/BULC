...
import { Trophy, BookOpen, Calculator, Download } from 'lucide-react';
...
const GPAResult: React.FC<GPAResultProps> = ({
  ...
  const handleDownloadPDF = () => {
    generateGPAPDF(courses, gpa, totalCredits, totalQualityPoints);
  };
      ...
      <div className="space-y-6">
        ...
        <div className="bg-gradient-to-br from-green-600/20 to-blue-600/20 border border-green-500/30 rounded-xl p-6 md:p-8 text-center">
          ...
          <button
              ...
              className="bg-green-600 hover:bg-green-700 text-white font-bold py-3 px-6 rounded-xl transition-all duration-200 flex items-center justify-center mx-auto transform hover:scale-105 shadow-lg"
          >
            <Download className="w-5 h-5 mr-2" />
            Download GPA Report (PDF)
          </button>
        </div>
      </div>
  );
